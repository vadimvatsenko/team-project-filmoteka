...
async function getSearchForm(genre = '', year = '', page = '', query = '') {
  ...
  const fetchCard = await axios.get(
    `${BASE_URL}${formSearch.discover}/movie?sort_by=popularity.desc${formSearch.genre}${formSearch.year}${formSearch.query}&include_adult=false&api_key=${KEY}&${formSearch.page}`
  );
...
//   formSearch.page = eventData.page;
...
function eventYear(evt) {
  ...
  if (evt) {
    ...
    if (localStorage.getItem('search')) {
      formSearch.query = JSON.parse(localStorage.getItem('search'));
    }
    pagination.on('afterMove', async function (eventData) {
      formSearch.page = eventData.page;
      getSearchForm(
...
function onResetSearch(evt) {
  evt.preventDefault();
  filterItem.genreForm.classList.remove('is-hidden');
